...
def course_details(request,kategory):
    ...
    print(kategory+"mmmmmmmmmmmmmmmmmmmmm")
#    list = list(Category.objects.all())
#    print(list)
    i=0

    books = Book.objects.filter(category=5)
#    books = Book.objects.all()

    cat=Category.objects.all()
#    print(cat)
#    while str(cat[i]) != str(kategory):
#        i+=1  
#    if cat[i] != kategory:
    j=1    
    ...
    books = Book.objects.filter(category=j)    
    return render(request,'courses.html', {'books': books})
...
#    cat = Category.objects.filter(author=request.user)
    ...
    return render(request,'videos.html', {'categories': self.categories})
