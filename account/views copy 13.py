...
from .models import Book, Category, Videos_sold, Buyer, News, Create_Watermark, Cart, User_Info, Profile, UserProfile
...
#        check_user = UserProfile.objects.get(user_id=14)
    ...
    #    videos = Book.objects.filter(category=j).filter(author=request.user)   
        if kategory == 'All':
            ...
            videos = Book.objects.filter(category=j).filter(watermark_ready=True)  
    #    print(videos) 
...
#    add_to_cart(request, 1, 1)
    ...
    for video in  videos:
        ...
        txt = TextClip("This is Watermark",fontsize=70,color='white')
