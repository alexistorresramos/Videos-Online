...
from .models import Book, Category, Videos_sold, Buyer, News, Create_Watermark, Cart, User_Info, Profile, UserProfile, Account
...
def modify_book(request, pk):
    if request.method == 'POST':
        ...
        return render(request,'modify_book.html', {'videos': videos})
    else:
        videos = Book.objects.filter(pk=pk)
        return render(request,'modify_book.html', {'videos': videos})
